...
const PageDetail = (argument) => {
  const preparePage = () => {
    ...
    const displayGame = (gameData) => {
      const { background_image, name, rating, rating_top, released, description, developers, parent_platforms, publishers, genres, tags, stores, ratings_count } = gameData;
      ...
      articleDOM.querySelector("h3.rating").innerHTML = `${rating}/${rating_top} - ${ratings_count} votes`;
      ...
      const screenshotsContainer = articleDOM.querySelector(".screen");
      const maxScreenshots = 4;
      let displayedScreenshots = 0;

      const getScreenshots = (argument) => {
        fetch(`https://api.rawg.io/api/games/${argument}/screenshots?key=${API_KEY}`)
          .then((response) => response.json())
          .then((responseData) => {
            responseData.results.forEach((element) => {
              if (displayedScreenshots < maxScreenshots) {
                screenshotsContainer.innerHTML += `<div class="col-6"><img src='${element.image}' class="screenshots"></div>`;
